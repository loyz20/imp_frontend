      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Draft', value: stats.draft ?? 0, color: 'from-slate-500 to-slate-600', icon: Clock },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <div>
              ...
              <AutocompleteInput
                ...
                fetchFn={(params) => goodsReceivingService.getAvailablePOs({ ...params, supplierId: form.supplierId || undefined })}
                getDisplayText={(po) => po.poNumber}
                renderItem={(po) => (
                  <div>
                    <p className="font-medium text-gray-800">{po.poNumber}</p>
                    <p className="text-xs text-gray-400">
                      {(typeof po.supplier === 'object' ? po.supplier?.name : '') || (typeof po.supplierId === 'object' ? po.supplierId?.name : '')}
            ...
            <div>
              ...
              <AutocompleteInput
                ...
                onChange={(text) => setForm((p) => ({ ...p, supplierName: text, supplierId: '' }))}
                onSelect={(supplier) => {
                  setForm((p) => ({
                    ...p,
                ...
                getDisplayText={(supplier) => supplier.name}
                renderItem={(supplier) => (
                  <div>
                    <p className="font-medium text-gray-800">{supplier.name}</p>
                    <p className="text-xs text-gray-400">{supplier.code || '-'} {supplier.city ? `· ${supplier.city}` : ''}</p>
                  </div>
                )}
                        ...
                        <AutocompleteInput
                          ...
                          onSelect={(prod) => {
                            ...
                            items[idx] = {
                              ...
                              price: Number(items[idx].price ?? 0),
                            };
                            setForm((p) => ({ ...p, items }));
                          }}
                          onClear={() => {
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900">{getReceivingDisplayNumber(receiving)}</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          <strong>{getReceivingDisplayNumber(receiving)}</strong> — {receiving.items?.length || 0} item
        </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">
          <strong>{getReceivingDisplayNumber(receiving)}</strong> akan dihapus. Tindakan ini tidak dapat dibatalkan.
        </p>
...
function getReceivingDisplayNumber(receiving) {
  return receiving?.receivingNumber || receiving?.grNumber || receiving?.invoiceNumber || '-';
}
